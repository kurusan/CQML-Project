...
public class Controller {
	...
	private Model model;
	private int a,b,c;
	private String regex = "^-?\\d{1,}$"; //Regex permettant de v�rifier si l'user � entrer un entier
	private Pattern motif ;              //le regex est compilt� et utilis� via un objet de type Pattern
	private String defaultLng = "fr"; //langage par d�faut
	//TextField
	@FXML
		...
		if(fOp.find() && sndOp.find() && thdOp.find()){
			try {
				a = Integer.parseInt(firstOp.getText());
				 if(a==0) {
					 AlertModal.showAlert(Alert.AlertType.ERROR, owner, "Form Error!","la valeur de a ne doit pas �tre nulle");
				 }
			...
			resolution(a,b,c);
			 ...
			 AlertModal.showAlert(Alert.AlertType.ERROR, owner, "Form Error!","Veuiller saisir des nombres ou v�rifier que tous les champs sont remplis");
			 return;
		}
	}
	 /**
	...
	public void handleTranslateButton(ActionEvent event){
		...
		if(lngButton.getText().equals("FR")) {
			...
			resultLabel.setText("R�sultat");
			submitButton.setText("R�soudre");
	...
	/**
	  ...
	  * Ouvre une nouvelle fen�tre et lui associe un controller
	 * @throws SQLException 
	  */
	@FXML
	...
	/**
	 * Ouvre une fen�tre d'aide lorsqu'on click sur le boutton help
	 * @param event
	 * @throws IOException
	 */
	@FXML
	...
	/**
	 * M�thode qui se charge de r�soudre une �quation du 2nd degr�
	 * @param a | b | c : int
	 * ********/
	...
	public void resolution(int a,int b, int c){
		...
		if(delta<0){
			 if(lngButton.getText().equals("ENG")){
			  ...
			  resultField.setText("Le syst�me n'admet pas de solutions dans R");
			  model.insertStatement(a+"x�"+"+("+b+")"+"x"+"+("+c+")","Le syst�me n'admet pas de solutions dans R", new Date());
			 return;
		}
